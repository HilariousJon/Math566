...
\begin{theorem}[\textbf{\href{https://en.wikipedia.org/wiki/Cauchy-Binet_formula}{Cauchy-Binet Theorem}}]
	\label{thm:cauchy-binet}
	Let \( A, B \) be \( n\times m \) and \( m\times n \) matrices, if \( n\leq m \), then:
	\[
		\det(AB) = \sum_{\abs{S} = n, S \subseteq [m]} \det(A[S]) \det(B^\intercal [S])
	\]
	If \( n> m \), then:
	\[
		\det(AB) = 0
...
\begin{theorem}[\textbf{Laplacian Expansion Theorem}]
	\label{thm:laplacian-expansion}
	Let \( X, Y \) be matrices of dimension \( n\times (n+m) \) amd \( m\times (n+m) \), then:
	\[
		\det \begin{pmatrix}
			X \\
			Y
		\end{pmatrix} = \sum_{S \subseteq \{1,\ldots, n+m\}, \abs{S} = n} \sign(S) \cdot \det(X[S]) \det(Y[\overline{S}])
	\]
	where \( \overline{S} = [n+m]-S \) and:
	\[
		\sign(S) = (-1)^{\# \{\overline{s} < s \; | \; s\in S, \; \overline{s} \in \overline{S}\}}
	\]
\end{theorem}

\begin{corollary}
	Let \( A, B \) be \( n\times m \) and \( m\times n \) matrcies respectively, with \( n\leq m \), then:
	\[
		\det \begin{pmatrix}
			0_{n\times n} & A               \\
			B             & \II_{m\times m}
		\end{pmatrix} = \sum_{\abs{S} = n} (-1)^n \det(A[S]) \det(B^\intercal[S])
	\]
\end{corollary}

\begin{proof}[\textbf{Proof of Cauchy-Binet Theorem \ref{thm:cauchy-binet}}]
	Recall that \( A, B \) has \( n\times m \) and \( m\times n \) dimension respectively. Notice that:
	\[
		\begin{pmatrix}
			AB & 0_{m\times m}   \\
			B  & \II_{m\times m}
	...
	\[
		\begin{aligned}
			\det(AB) & = (-1)^n \cdot 1 \cdot \det
			\begin{pmatrix}
				0_{n\times n} & A               \\
				B             & \II_{m\times m}
			\end{pmatrix} = (-1)^{2n} \sum_{\abs{S} = n }\det(A[S]) \det(B^\intercal [S]) \\
			         & = \sum_{\abs{S} = n} \det(A[S]) \det(B^\intercal [S])
		\end{aligned}
	\]
\end{proof}

\begin{definition}
	A Hermitian matrix \( \bA \in \CC_{n\times n} \) is \underline{positive semidefinite} if for every
	\( x \in \CC^n \), have \( x^* \bA x \geq 0 \).
\end{definition}

\begin{theorem}
	\( \bA \) is positive semidefinite if and only if all eigenvalues of \( \bA \) are \( \geq 0 \).
\end{theorem}

\begin{lemma}
	Let \( \bA \) be an \( n\times m \) matrix, then \( \bA \bA^* \in \CC_{n\times n} \) is positive
	semidefinite.
\end{lemma}
...
\begin{definition}[\textbf{Unitary Matrix}]
	A complex square matrix \( U \) is \underline{unitary} if \( U^* = U^{-1} \).
\end{definition}

\begin{definition}[\textbf{Orthogonal Matrices}]
	An orthogonal matrix is a real square matrix \( O \) whose rows and cols are \textbf{orthonormal}, in particular equivalent to:
	\[
		O^\intercal = O^{-1}
	\]
\end{definition}

\begin{theorem}[\textbf{Spectral Theorem for Hermitian Matrices}]
	\label{thm:spectral}
	A complex (real) square matrix \( A \) is Hermitian (real symmetric). Then there exists a unitary (orthogonal) \( U \) and a real diagonal matrix \( \Lambda \), s.t.
	\[
		A = U \Lambda U^{-1}
	\]
