...
\begin{proof}
	...
	\begin{itemize}
		...
		\item If \( S \) did forms a spanning tree. By degree handshake theorem, one can see that the tree always have a leave vertex, namely a vertex that only connects with a unique edge. We now choose a leave node \( v \) in the spanning tree and the corresponding edge \( e \), by definition of incidence matrix, for the row represented by \( v \) attains only \( (v,e) \)-entry which is non-zero, and it will be \( \pm 1 \). By property of determinant, abuse the notation of \( S \) for the whole spanning tree, see that \( \det(S) = \det(S \backslash \{v\}) \). This is true as we also deleted the corresponding edges as we deleted the vertex. And we induct on the leavenode to reduct on the whole graph, yields the result to be \( \pm 1 \).
	\end{itemize}
\end{proof}
...
\begin{theorem}[\textbf{Matrix-Tree Theorem}]
	...
	\]
	where \( L_{r,c} \) is the \( (r,c) \)-cofactor of \( \LL(G) \). Note that the following prove only completes for \( (v,v) \)-cofactor, and combined with \textbf{Lemma} \ref{lem:cofactor-same} the prove completes.
\end{theorem}
